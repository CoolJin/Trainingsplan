...
export function CardStack<T extends CardStackItem>({
    ...
    // Sync active state if initialIndex prop explicitly updates (optional, but good for date changes)
    React.useEffect(() => {
        setActive(wrapIndex(initialIndex, len));
    }, [initialIndex, len]);
    ...
    return (
        ...
        >
            {/* Stage */}
            <div
                className="relative w-full flex justify-center perspective-container"
                style={{ height: Math.max(380, cardHeight + 80), outline: 'none' }}
                tabIndex={0}
                onKeyDown={onKeyDown}
            >
                {/* background wash / spotlight (unique feel) */}
                <div
                    className="pointer-events-none absolute inset-x-0 top-6 mx-auto h-48 w-[70%] max-w-lg rounded-full bg-pink-500/10 blur-3xl"
                    aria-hidden="true"
                />
                <div
                    className="pointer-events-none absolute inset-x-0 bottom-0 mx-auto h-40 w-[76%] rounded-full bg-blue-500/10 blur-3xl"
                    aria-hidden="true"
                />
                        ...
                        {items.map((item, i) => {
                            ...
                            return (
                                <motion.div
                                    key={item.id}
                                    className={cn(
                                        "absolute bottom-0 rounded-2xl border border-white/10 overflow-hidden shadow-2xl bg-zinc-900",
        ...
        <div className="relative h-full w-full bg-zinc-900">
            ...
            <div className="absolute inset-0">
                {item.imageSrc ? (
                    ...
                    <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-zinc-800 to-black text-sm text-zinc-500">
